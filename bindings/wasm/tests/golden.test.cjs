...
// file is the exact cross-language parity check — the same assertion the Rust,
// Python, Node, Go, C# , Java and R suites make against the same file.
//
// This is the check the WASM binding was missing: it is the reach the whole web
// renderer stands on, and it was the only one of the eight with no test at all.
