// Throughput benchmark for the wickra-terminal Node binding.
//
...
// out -- so the number here is the cost of crossing the Node/Rust boundary once
// per command, plus building and parsing the JSON on each side. It is not a
// speed claim and not a cross-library ratio: the other side of every one of
// these calls is the same code.
//
// The Rust core runs the identical loop with no boundary at all, as
// examples/rust --bin throughput. That is the floor.
//
// Build the binding first (`npm run build`), then from bindings/node:
//
//     node benchmarks/throughput.js
//     node benchmarks/throughput.js --ticks 100000
...
function main() {
  ...
  console.log(`wickra-terminal Node throughput - ${n(ticks)} commands (median of 3)\n`);
