...
- **The boundary is data, not an API.** A config JSON in, a command JSON in, a
  frame JSON out. That is why 421 indicators became reachable from ten languages
...
indicators, and the registry offers 421. See [BENCHMARKS.md](BENCHMARKS.md).
...
registry suite that constructs and drives all 421 indicators, and a test that
