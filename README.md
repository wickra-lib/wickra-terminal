...
tape and <!--indicator-count-->461<!--/indicator-count--> streaming indicators — with a native **TUI** and a **Web** front-end
...
- **The boundary is data, not an API.** A config JSON in, a command JSON in, a
  frame JSON out. That is why <!--indicator-count-->461<!--/indicator-count--> indicators became reachable from ten languages
...
indicators, and the registry offers <!--indicator-count-->461<!--/indicator-count-->. See [BENCHMARKS.md](BENCHMARKS.md).
...
registry suite that constructs and drives all <!--indicator-count-->461<!--/indicator-count--> indicators, and a test that
